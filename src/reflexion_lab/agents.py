...
class BaseAgent:
    ...
    def run(self, example: QAExample) -> RunRecord:
        ...
        for attempt_id in range(1, self.max_attempts + 1):
            from .mock_runtime import METRICS, reset_metrics
            reset_metrics()
            ...
            if self.agent_type == "reflexion" and judge.score == 0 and attempt_id < self.max_attempts:
                ...
                reflection_memory.append(
                    f"Attempt {attempt_id} failed because: {ref.failure_reason}. "
                    f"Lesson learned: {ref.lesson}. "
                    f"Strategy for next attempt: {ref.next_strategy}"
                )
            
            if METRICS["tokens"] > 0:
                token_estimate = METRICS["tokens"]
